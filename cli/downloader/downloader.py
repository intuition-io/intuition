...
class Quote:
    ...
    def get_infos(self, action, save_quote):
        ...
        if ( save_quote ):
            ...
            save_fd = open(save_f, 'w')
            if ( action == 'raw' ):
                save_fd.write('Date Hour Close High Low Open Volume\n')
        ...
        if ( save_quote and action == 'current'):
                save_fd.write(date + ' ' + str(close) + ' ' + str(high) + ' ' + str(low) + ' ' + str(self.datum['open']) + ' ' + str(volume) + '\n')
...
def main():
    ...
    stocks = args.output.split(',')

    for stock in stocks:
        q = Quote(q_parser, stock, int(q_parser.get(stock, 'days')), int(q_parser.get(stock, 'precision')));
        q.get_infos(q_parser.get(stock, 'action'), True)
