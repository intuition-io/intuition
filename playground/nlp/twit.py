...
#TODO General implementation: twitter collector according words 
#     stored in database (key | word | freq | sentiment stats | relevant score)
#TODO Changing from sqlite3 mysql
#TODO Sentiment analysis: Bayesian naive classifier with nltk

#TODO read it in configuration file
username = 'XavierBruhiere'
password = 'quantrade'
...
for row in wordListCsv:
    ...
    word_filter.append(row[0])

print "Filtering the following words: ", ', '.join(word_filter)
...
try:
    ...
    with tweetstream.FilterStream(username, password, track=word_filter) as stream:
        for tweet in stream:
            tweettimestamp = time.mktime(time.strptime(
                tweet['created_at'], "%a %b %d %H:%M:%S +0000 %Y")) - time.timezone

            print (stream.count, "(", stream.rate, "tweets/sec). ",
