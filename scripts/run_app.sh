...
if [ $choice == 1 ]; then
    time $QTRADE/application/app.py --initialcash 50000 --tickers random,20 \
        --loglevel INFO --algorithm BuyAndHold --manager Constant --start 2012-05-10 \
        --frequency daily --database backtest --exchange nasdaq
...
# Forex live test !
    ...
    time $QTRADE/application/app.py --initialcash 50000 --tickers EUR/USD,EUR/GBP,GBP/USD,EUR/JPY,USD/CAD,AUD/USD,GBP/JPY \
        --algorithm VWAP --manager Constant --end 23h \
        --database liveforex --exchange forex --frequency minute --live

# Equitie live test !
elif [ $choice == 3 ]; then
    time $QTRADE/application/app.py --initialcash 50000 --tickers random,20 \
        --algorithm BuyAndHold --manager Fair --end 22h \
        --database live-equities --exchange paris --frequency minute --live
